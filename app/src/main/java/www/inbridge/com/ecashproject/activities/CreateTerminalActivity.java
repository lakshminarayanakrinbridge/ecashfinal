...
public class CreateTerminalActivity extends AppCompatActivity {
    ...
    public void onClickSubmitButton(View v) {
        ...
        if (mobilenumberEdittext.getText().toString().trim().length() != 10) {
            ...
            editor.putString(Sharedpref.KEY_CASHIERNAME1, cashiernameEdittext.getText().toString().trim());
            editor.putString(Sharedpref.KEY_CASHIERMOBILE1, mobilenumberEdittext.getText().toString().trim());
            editor.putString(Sharedpref.KEY_CASHIEREMAIL1, emailidEdittext.getText().toString().trim());
            editor.commit();
            onBackPressed();
            Toast.makeText(getApplicationContext(), sharedPreferences.getString(Sharedpref.KEY_CASHIERNAME1, "cashier1"), Toast.LENGTH_SHORT).show();
